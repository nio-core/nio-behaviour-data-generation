...
public class CheckerTest implements IController, GameBoard.CheckMateListener {

    @Test
    public void start() {
        GameBoard gameBoard = new GameBoard("startpos", GameType.Checkers, this);
        EngineController testEngineController = EngineController.getFromString("pondes", GameType.Checkers, gameBoard);

        Assert.assertNotNull("TestEngine should not be null", testEngineController);
        Assert.assertEquals("Wrong type", "ponder", testEngineController.getType().name());
